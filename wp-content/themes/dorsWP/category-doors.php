  ...
  <?php
    ...
    ?>
        ...
        <?php
          ...
          // Проверяем, есть ли у нас записи на странице
          if(have_posts()){
            // И до тех пор, пока есть записи, мы их будем выводить
            while(have_posts()){
              the_post();
              // Теперь можем получать доступ к нашей записи
              // Получаем категорию
              $all_category = get_the_category();
              // Флаг нужной нам категории
              $res_name = '';
              foreach($all_category as $category){
                // Нужно проверить запись на соответствующую категорию и вывести только ее
                // Это у нас глухие, со стеклом и увеличенного формата, поэтому посмотрим для них ID (term_id)
                if($category->term_id == 14 || $category->term_id == 15 || $category->term_id == 16){
                  // Если запись попадает под одну из этих категорий, то она будет выводиться (ее slag - ярлык),
                  // который нам нужен для фильтрации.
